...
class Student:
    def __init__(self, root):
        ...
        # Header
        heading = Label(self.root, text="Thông tin sinh viên", font=CAFE_FONT_TITLE, bg=CAFE_PRIMARY, fg=CAFE_WHITE,
                        bd=0, relief=FLAT)
        ...
        label_Update_att = Label(Left_frame, bg=CAFE_ACCENT, fg=CAFE_WHITE, text="Thông tin sinh viên",
                                 font=("Segoe UI", 16, "bold"))
        label_Update_att.place(x=0, y=1, width=720, height=44)

        # Course info
        current_course_frame = LabelFrame(Left_frame, bd=0, bg=CAFE_BG, fg=CAFE_TITLE, text="Thông tin khoá học",
                                          font=("Segoe UI", 12, "bold"), highlightbackground=CAFE_PRIMARY,
        ...
        dep_combo = ttk.Combobox(current_course_frame, textvariable=self.var_dep, font=CAFE_FONT, state="readonly",
                                 width=20)
        dep_combo["values"] = (
        "Chọn chuyên ngành", "Điện tử viễn thông", "IT", "Cơ khí", "Điện", "Kế toán", "Tự động hóa")
        dep_combo.current(0)
        ...
        course_combo = ttk.Combobox(current_course_frame, textvariable=self.var_course, font=CAFE_FONT,
        ...
        year_combo = ttk.Combobox(current_course_frame, textvariable=self.var_year, font=CAFE_FONT, state="readonly",
                                  width=20)
        ...
        semester_combo = ttk.Combobox(current_course_frame, textvariable=self.var_semester, font=CAFE_FONT,
                                      state="readonly", width=20)
        ...
        # Class & Student info
        class_student_frame = LabelFrame(Left_frame, bd=0, bg=CAFE_BG, fg=CAFE_TITLE, text="Thông tin lớp học",
                                         font=CAFE_FONT_BOLD, highlightbackground=CAFE_PRIMARY, highlightthickness=1)
        class_student_frame.place(x=8, y=170, width=710, height=450)

        # Student ID
        studentID_label = Label(class_student_frame, text="ID Sinh Viên:", font=CAFE_FONT_BOLD, bg=CAFE_BG,
                                fg=CAFE_TITLE)
        studentID_label.grid(row=0, column=0, padx=12, pady=6, sticky=W)
        studentID_entry = ttk.Entry(class_student_frame, width=20, textvariable=self.var_std_id, font=CAFE_FONT,
                                    state="disabled")
        studentID_entry.grid(row=0, column=1, padx=10, pady=6, sticky=W)
        studentName_label = Label(class_student_frame, text="Tên Sinh Viên:", font=CAFE_FONT_BOLD, bg=CAFE_BG,
                                  fg=CAFE_TITLE)
        ...
        gender_combo = ttk.Combobox(class_student_frame, textvariable=self.var_gender, font=CAFE_FONT, state="readonly",
                                    width=18)
        ...
        self.dob_entry = DateEntry(class_student_frame, width=18, bd=2, selectmode='day', year=2021, month=5,
                                   font=CAFE_FONT, day=22, date_pattern='dd/mm/yyyy')
        ...
        save_btn = Button(btn_frame, text="Save", command=self.add_data, font=CAFE_FONT_BOLD, bg=CAFE_BUTTON,
                          fg=CAFE_BUTTON_TEXT, width=14)
        save_btn.grid(row=0, column=0, padx=6)
        update_btn = Button(btn_frame, text="Edit", command=self.update_data, font=CAFE_FONT_BOLD, bg=CAFE_BUTTON,
                            fg=CAFE_BUTTON_TEXT, width=14)
        update_btn.grid(row=0, column=1, padx=6)
        delete_btn = Button(btn_frame, text="Delete", command=self.delete_data, font=CAFE_FONT_BOLD, bg=CAFE_BUTTON,
                            fg=CAFE_BUTTON_TEXT, width=14)
        delete_btn.grid(row=0, column=2, padx=6)
        reset_btn = Button(btn_frame, text="Reset", command=self.reset_data, font=CAFE_FONT_BOLD, bg=CAFE_BUTTON,
                           fg=CAFE_BUTTON_TEXT, width=14)
        ...
        take_photo_btn = Button(btn_frame1, text="Lấy ảnh sinh viên", command=self.generate_dataset,
                                font=CAFE_FONT_BOLD, bg=CAFE_SUCCESS, fg=CAFE_WHITE, width=28)
        take_photo_btn.grid(row=0, column=0, padx=6)
        update_photo_btn = Button(btn_frame1, text="Training Data", command=self.train_classifier, font=CAFE_FONT_BOLD,
                                  bg=CAFE_ACCENT, fg=CAFE_WHITE, width=28)
        update_photo_btn.grid(row=0, column=1, padx=6)

        # THÊM NÚT CHUYỂN SANG TRANG NHẬN DIỆN KHUÔN MẶT (ĐÃ SỬA LỖI EMOJI)
        btn_frame2 = Frame(class_student_frame, bg=CAFE_BG)
        btn_frame2.place(x=0, y=360, width=705, height=38)
        face_recognition_btn = Button(btn_frame2, text=">> Điểm danh",
                                      command=self.open_face_recognition,
                                      font=CAFE_FONT_BOLD, bg=CAFE_DANGER, fg=CAFE_WHITE, width=58)
        face_recognition_btn.grid(row=0, column=0, padx=6, columnspan=2)
        ...
        search_combo = ttk.Combobox(search_frame, font=CAFE_FONT, state="readonly", width=13,
        ...
        search_btn = Button(search_frame, text="Search", font=CAFE_FONT_BOLD, bg=CAFE_BUTTON, fg=CAFE_BUTTON_TEXT,
                            width=10, command=self.search_data)
        search_btn.grid(row=0, column=3, padx=4)
        showAll_btn = Button(search_frame, text="ALL", font=CAFE_FONT_BOLD, bg=CAFE_BUTTON, fg=CAFE_BUTTON_TEXT,
        ...
        "id", "dep", "course", "year", "sem", "name", "div", "roll", "gender", "dob", "email", "phone", "address",
        ...
        for name, text in [
            ("id", "ID Sinh viên"), ("dep", "Chuyên ngành"), ("course", "Chương trình học"), ("year", "Năm học"),
            ("sem", "Học kì"),
            ("name", "Họ tên"), ("div", "Lớp biên chế"), ("roll", "CMND"), ("gender", "Giới tính"),
            ("dob", "Ngày sinh"),
        ...
        label_studentsb = Label(Underright_frame, bg=CAFE_ACCENT, fg=CAFE_WHITE, text="Danh sách lớp học",
                                font=CAFE_FONT_BOLD)
        ...
        addTc_btn = Button(btn_framestd, text="Add", command=self.add_Classdata, font=CAFE_FONT_BOLD, bg=CAFE_BUTTON,
                           fg=CAFE_BUTTON_TEXT, width=10)
        addTc_btn.grid(row=0, column=0, padx=6, pady=6)
        deleteTc_btn = Button(btn_framestd, text="Delete", command=self.delete_Classdata, font=CAFE_FONT_BOLD,
                              bg=CAFE_BUTTON, fg=CAFE_BUTTON_TEXT, width=10)
        deleteTc_btn.grid(row=0, column=1, padx=6, pady=6)
        updateTc_btn = Button(btn_framestd, text="Update", command=self.update_Classdata, font=CAFE_FONT_BOLD,
                              bg=CAFE_BUTTON, fg=CAFE_BUTTON_TEXT, width=10)
        updateTc_btn.grid(row=0, column=2, padx=6, pady=6)
        resetTc_btn = Button(btn_framestd, text="Reset", command=self.reset_Classdata, font=CAFE_FONT_BOLD,
                             bg=CAFE_BUTTON, fg=CAFE_BUTTON_TEXT, width=10)
        ...
        self.StudentTable = ttk.Treeview(tablestd_frame, column=("class", "name"), xscrollcommand=scroll_x.set,
    ...
    def open_face_recognition(self):
        """Chuyển từ trang quản lý sinh viên sang trang nhận diện khuôn mặt"""
        try:
            # Import face_recognition3 module
            from face_recognition3 import Face_Recognition

            # Ẩn cửa sổ hiện tại
            self.root.withdraw()

            # Tạo cửa sổ mới cho face recognition
            new_root = Toplevel()
            new_root.protocol("WM_DELETE_WINDOW", lambda: self.on_face_recognition_close(new_root))

            # Khởi tạo face recognition
            Face_Recognition(new_root)

        except ImportError:
            messagebox.showerror("Lỗi",
                                 "Không thể tải module nhận diện khuôn mặt!\nVui lòng kiểm tra file face_recognition3.py",
                                 parent=self.root)
        except Exception as e:
            messagebox.showerror("Lỗi", f"Đã xảy ra lỗi: {str(e)}", parent=self.root)

    def on_face_recognition_close(self, face_window):
        """Xử lý khi đóng cửa sổ face recognition"""
        face_window.destroy()
        self.root.deiconify()  # Hiển thị lại cửa sổ quản lý sinh viên
        ...
        try:
            ...
            if self.var_dep.get() == "Chọn chuyên ngành" or self.var_std_name.get() == "" or self.var_std_id.get() == "" or self.var_div.get() == "":
                ...
                try:
                    ...
                    # SAU KHI THÊM THÀNH CÔNG, HỎI NGƯỜI DÙNG CÓ MUỐN CHUYỂN SANG TRANG NHẬN DIỆN KHÔNG
                    answer = messagebox.askyesno("Chuyển trang",
                                                 "Thêm sinh viên thành công!\nBạn có muốn chuyển sang trang nhận diện khuôn mặt không?",
                                                 parent=self.root)
                    if answer:
    ...
    def update_data(self):
        if self.var_dep.get() == "Chọn chuyên ngành" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                Update = messagebox.askyesno("Update", "Bạn có muốn cập nhật thông tin sinh viên này không?",
                                             parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(host='localhost', user='root', password='',
                                                   database='face_recognizer', port='3306')
    ...
    def reset_data(self):
        ...
        self.dob_entry.set_date("22/05/2021"),
        self.var_email.set(""),
        self.var_phone.set(""),
